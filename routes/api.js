...
// Gamertag endpoint
router.get('/v1/gamertag/:gamertag', cache('2 minutes'), async (req, res) => {
    request.requestAPIHandler(request.BASE_API_URL + request.FRIENDS + req.params.gamertag, res)
})

// Xuid endpoint
router.get('/v1/xuid/:xuid', cache('2 minutes'), async (req, res) => {
    request.requestAPIHandler(request.BASE_API_URL + request.ACCOUNT + req.params.xuid, res)
})

// Fuuid endpoint
router.get('/v1/fuid/:fuuid', cache('2 minutes'), async (req, res) => {
